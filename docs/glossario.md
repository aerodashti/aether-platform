...
| Lançamento de custo | `custo` | Lançamento | `despesa`, `gasto`, `expense` | Uma despesa de uma aeronave, classificada em tipo e categoria, atribuída a um proprietário ou rateada entre todos. O valor gravado é sempre BRL. |
...
| Câmbio | `cambio` | Câmbio do dia | `cotacao`, `taxa`, `fx` | A cotação usada num lançamento em moeda estrangeira, gravada no ato. O BRL derivado nunca muda depois. |
